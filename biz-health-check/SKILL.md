...
description: 사업자등록번호 하나로 "이 사업자, 실제 문제 없나"를 확인한다 — 국세청 사업자등록 상태·국민연금 가입 사업장·국세 체납 명단·금융위 법인개요·지방행정 인허가 영업상태를 무료 공공 데이터로 교차 조회해 사실만 병렬하는 실사 리포트(점수·등급·위험 판정 없음).
license: MIT
metadata:
  category: business
  locale: ko-KR
...
사업자등록번호(+상호/지역)를 입력하면 무료 공공 데이터 5종을 한 번에 교차 조회해 실사 리포트 한 장을 만든다. 같은 레포의 단품 스킬 helper를 그대로 재사용한다(단일 진실원천).
...
- "○○○-○○-○○○○○ 살아있는 회사야? 직원은 좀 있고, 체납 이력은 없어?"

## Prerequisites

- 인터넷 연결, `python3`
- 같은 레포의 단품 스킬 5종(이 복합이 helper를 재사용)
- proxy 섹션을 켜려면 Lily Box proxy 접근 가능

## Credential requirements

- 사용자 측 필수 시크릿 없음.
- proxy 섹션(국세청 상태·국민연금·금융위)은 Lily Box proxy 서버의 `DATA_GO_KR_API_KEY`로 동작한다. 활용신청 항목은 각 단품 스킬 문서를 따른다.
- 무인증 섹션(체납·인허가)은 키 없이 사용자 머신에서 직접 동작한다.

## Inputs

- `b_no`: 사업자등록번호 10자리(하이픈 허용) — 상태조회에 필요
...
- `sections`: 5개 섹션 각각의 `data`(단품 응답 원문) 또는 `status: unavailable` + `note`
